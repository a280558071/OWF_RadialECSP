...
y=binvar(L,L_c,'full');   %Vars for line operation flag in different contigencies, y(line operation,Cont_l)==0
y_ij=binvar(L,L_c,2,'full');  %Vars for parent-child relationship of line l (i→j)
Pij=sdpvar(L,L_c,'full');    %Vars for active power flow in each line in Contigency L_c+Normal Condition, unit: p.u. 
Pw_shed=sdpvar(WTs,L_c, 'full'); % shedded active wind power 
g_Sub_P=sdpvar(Ns,L_c,'full');    %Vars for generated power of Subs
%% ***********Objective Function***********
Obj_inv=sum(Cost.*x)*1e4;
...
for l=1:length(Cr_Cab)
    Cons_CAC=[Cons_CAC,x(Cr_Cab(l,1))+x(Cr_Cab(l,2))<=1];
end
Cons=[Cons,Cons_CAC];
...
ops=sdpsettings('solver','cplex','verbose',2,'usex0',0,'Cplex.Benders.Strategy',0,'cplex.timelimit',10000); %,'gurobi.MIPGap',5e-2,,'Gurobi.MIPFocus',3,'gurobi.MIPGap',1e-3,'gurobi.TimeLimit',30000,
% ops=sdpsettings('solver','gurobi', 'gurobi.MIPGap',1e-4,'verbose',2,'gurobi.LogFile',['Case_',num2str(WTs),'_LDF.log']);%,'gurobi.Cuts',0,'usex0',1,
%% solve the problem
...
C_EENG=abs(Pr_ele*2000*20*0.0045*(len_l*(s_x.*s_Pij)));
